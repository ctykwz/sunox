...
`sunox login` first tries to read the Clerk auth cookie from Chrome, Arc, Brave, Firefox, or Edge. On Windows it skips live Chromium cookie databases so it never force-closes a running browser to unlock App-Bound cookies; Firefox is read through its non-destructive read-only SQLite path. If no reusable session is found, the dedicated interactive-login fallback requires an installed Chromium-family browser. When extraction succeeds, Sunox links the session to the exact local profile/channel, reads public profile settings, and probes the same installed browser binary for its runtime user-agent and client hints without opening a visible window or navigating to Suno. Legacy auth files missing this context are repaired on their next authenticated command. Newly recovered fields take priority, stored values are retained field-by-field when a probe cannot refresh them, and built-in constants are only used when neither source has a usable value. The same context is used for Clerk login/JWT refresh and Suno API requests.
...
| `--lyrics` / `--lyrics-file` | Custom lyrics with `[Verse]` tags | `max_lengths.prompt` |
| `--prompt` (describe) | Free text description | `max_lengths.gpt_description_prompt` |
...
| Single clip | `GET /api/clip/{id}` | Current Web primary read used by polling and source-dependent operations |
...
| Aligned lyrics | **v3** (`POST`, then `GET /api/gen/{id}/aligned_lyrics/v3`) | Current Web primary; v2 is an explicit compatibility fallback only |
...
| Persona trash/restore/purge | `PUT /api/persona/trash-persona/{id}/?undo=...&hide=...` | Current bundle-confirmed per-ID route |
| Playlist list | `GET /api/playlist/me` | Bundle-confirmed |
| Playlist detail | `GET /api/playlist/v2/{id}` | Bundle-confirmed |
| Playlist mutation | `POST /api/playlist/create/`, `PATCH /api/playlist/v2/{id}`, `POST/DELETE /api/playlist/v2/{id}/save`, `POST /api/playlist/v2/{id}/tracks/{add,remove}`, `POST /api/playlist/v2/{id}/tracks/reorder-by-index`, `POST /api/playlist/v2/{id}/trash`, `POST /api/playlist_reaction/{id}/update_reaction_type/` | Current routes are primary; legacy `POST /api/playlist/set_metadata` is used only for arbitrary external cover URLs that v2 cannot represent |
...
| Image upload | `POST /api/uploads/image/`, presigned S3 form upload, `POST /api/uploads/image/{id}/upload-finish/` | CLI workflow implemented for clip and playlist covers; playlist cover patch uses `PATCH /api/playlist/v2/{id}` with `cover_url`, `cover_image_s3_id`, `cover_is_user_set`; clip cover patch uses `POST /api/gen/{id}/set_metadata/` with `image_s3_id` for uploaded images and `image_url` only for external URLs |

Generation tasks use `/api/generate/v2-web/`. The create payload was refreshed from Suno's deployed web bundle on July 19, 2026: custom lyrics are sent in `prompt` with `metadata.create_mode="custom"`; simple description mode sends the description in `gpt_description_prompt`, leaves `prompt` empty, and uses `metadata.create_mode="simple"` plus `metadata.lyrics_model="default"`. A solved challenge token carries the provider reported by the current web preflight. Sunox resolves `metadata.user_tier`, account model availability, the account default, and field limits from `/api/billing/info/`; `default_model=auto` falls back to v5.5 only when that read is unavailable. With `--enhance-tags`, Sunox first calls `/api/prompts/upsample` and includes current custom lyrics as context for vocal requests, carries the returned tags plus `request_id` into `metadata.last_tags_generation`, and marks `override_fields=["tags"]`; the `personalization_enabled` field follows the captured web submit shape. Without that flag it omits `metadata.last_tags_generation`. Instrumental create also uses custom mode; when `sunox create --instrumental <prompt>` is used, the prompt is folded into style tags and the submitted `prompt` field stays empty. `sunox clip inspire` implements the live-captured `task: "playlist_condition"` flow for one source clip: it upsamples the supplied tags with the lyrics as context, puts lyrics in `prompt`, carries the real upsample metadata, and does not expose unverified instrumental or multi-source variants. Cover uses current `task="cover"` with `metadata.create_mode="custom"`; extend sets `metadata.lyrics_updated` only when replacement lyrics were supplied. Extend fetches the source clip through `GET /api/clip/{id}` before submit, uses a feed/v3 exact-id metadata enrichment only when that current response lacks source style fields, sends `title` as the source title unless `--title` is provided, defaults `tags` and `negative_tags` from the source when available, and inherits `metadata.make_instrumental` unless `--instrumental` or `--no-instrumental` overrides it; use `--tags` and `--exclude` to override the inherited values. `clip list` and multi-clip generation polling use `POST /api/feed/v3`; a single clip uses `GET /api/clip/{id}`. List exposes query-only filters such as `--liked`, `--public`, `--upload`, `--cover`, `--extend`, and `--sort popular`; this is not a library sync workflow. Timed lyrics use the v3 start/poll contract and fall back to v2 only when the source has no lyrics to submit or v3 is unavailable. Remaster uses the live-captured `/api/generate/upsample` route, speed adjust uses `/api/clips/adjust-speed/`, reverse uses `/api/clips/reverse-clip/`, and crop/fade use `/api/edit/*` action routes. Concat completed in a live CLI validation from an original generation source; edited inputs can be rejected by Suno with `Bad history.` Commands that submit through `/api/generate/v2-web/` preflight `/api/c/check` with `ctype=generation`; if Suno reports a challenge and the stored Clerk session can refresh, Sunox refreshes the JWT once and repeats the preflight. A remaining challenge is solved silently with hCaptcha/provider 1 or Cloudflare Turnstile/provider 2 according to `captcha_version`, prioritizing stored verified account cookies and the matching recorded browser source. When no challenge remains they submit without a challenge token. Use `--token <solved>` for an external token, `--captcha` to force verification, or `--no-captcha` to disable automatic verification on create, inspire, cover, extend, and stems. Audio upload initialization sends the current flat `{extension,is_stem_mix,upload_type}` body, and upload finish sends `agreed_to_vip_upload_terms`; the rest of the `file_upload` workflow was live-verified. Current cover, concat, visibility, upload, playlist, and metadata bodies are bundle-aligned and contract-tested. Playlist remove intentionally submits one clip per request because larger live batches can return Suno 500s. The production-live `/api/clips/get_similar/` and lyrics-only `POST /api/generate/lyrics/` capabilities remain supported as supplemental service endpoints; neither has a behaviorally equivalent replacement in the current Web bundle.

Authenticated API requests mirror Suno Web's public request context: bearer JWT, dynamic `Browser-Token`, browser-derived user-agent/language/client hints, Fetch Metadata, origin/referer, and the matching browser `Device-Id` when available. Legacy auth recovers missing values from the same-account browser profile and preserves stored values when probing fails. Sunox never invents an all-zero device identity and does not fabricate conditional `Referring-*` marketing headers without real navigation/referrer context; `Session-Id` is response metadata in the current Web client, not a common request header.
