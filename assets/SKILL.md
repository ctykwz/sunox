...
- for unattended generation that must not add a Suno tab to the active browser
  ...
  connect. The frame is credentialless, stops and replaces the host response at
  `document_start`, and connects only after its network-visible nonce plus final
  request and controlled response headers are verified. Unexpected navigation,
  caching, reload, disconnect, protocol drift, or any failed identity check removes
  the frame and fails closed; there is no visible or credentialed fallback. The same
