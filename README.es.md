...
`sunox login` primero intenta leer la cookie Clerk desde Chrome, Arc, Brave, Firefox o Edge. En Windows evita las bases activas de Chromium para no activar el desbloqueo App-Bound ni cerrar el navegador, pero Firefox se lee mediante SQLite de solo lectura y sin efectos destructivos. Si no hay una sesión reutilizable, el perfil interactivo dedicado requiere un navegador de la familia Chromium. Sunox vincula la sesión con el perfil y canal exactos y consulta el mismo binario para obtener user-agent, idiomas y Client Hints sin ventana adicional ni acceso a Suno. Los valores nuevos tienen prioridad, los anteriores se conservan campo por campo si falla la sonda y las constantes integradas solo se usan como último recurso para Clerk y la API de Suno.
