...
pub async fn agent_info(_ctx: &AppContext) -> Result<(), CliError> {
    ...
    let mut info = serde_json::json!({
        ...
        "command_notes": {
            "create": {
                "default_challenge": "preflights POST /api/c/check with ctype=generation; if Suno reports a challenge and stored Clerk refresh material exists, refreshes the JWT once and repeats the preflight; when still required, challenge_browser=auto first asks the paired Sunox Browser Bridge to create one nonce-bound Suno iframe inside Chrome's invisible offscreen document. The frame executes the silent challenge without creating a tab, popup, minimized window, or separate browser process and is removed on every terminal path. Only allowlisted error codes cross extension boundaries; raw provider values are never forwarded. A configured but unavailable Bridge fails closed; auto falls back to an isolated matching browser only when no Bridge pairing exists. It then submits provider 1 for hCaptcha or provider 2 for Cloudflare Turnstile; when no challenge is required, token and token_provider are serialized as null to match the current Web client",
                "challenge_flags": {
                ...
                "enhance_tags": "pass --enhance-tags only when the user wants Suno to enhance style tags; Sunox first verifies that the resolved model has the custom badge or tag_upsample feature, then calls /api/prompts/upsample with current custom lyrics as context for vocal requests, validates the returned tags against the model length limit, carries the returned tags plus request_id into metadata.last_tags_generation, and marks override_fields=[\"tags\"]; personalization_enabled follows the captured web submit shape",
