...
pub async fn agent_info(_ctx: &AppContext) -> Result<(), CliError> {
    ...
    let info = serde_json::json!({
        ...
        "agent_safety": {
            ...
            "captcha": "for unattended no-new-window generation, use confirmed Browser Bridge installation as the command-selection boundary: when installed, omit --no-captcha and use challenge_browser=existing, which verifies that a refreshed authenticated suno.com tab is connected and fails closed when it is not; when the bridge is absent or installation is unknown, keep --no-captcha so a required challenge stops before submission. The default challenge_browser=auto may open an isolated browser when no paired tab responds. Do not force --captcha unless the user explicitly asks, and prefer an externally supplied --token when provided",
