...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct InstallRuntimeState {
    pub(crate) reload_required: Option<bool>,
    pub(crate) runtime_ack_pending: bool,
    pub(crate) pending_origin: Option<PendingActivation>,
}

impl InstallOutcome {
    pub(crate) fn status(self, runtime_state: InstallRuntimeState) -> &'static str {
        match self {
...
pub(crate) struct InstallReport {
    ...
    pub(crate) runtime_state: InstallRuntimeState,
    pub(crate) activation_required: Option<&'static str>,
    pub(crate) activation_options: Vec<&'static str>,
    pub(crate) next_steps: Vec<&'static str>,
}

pub(crate) async fn install_with_probe(
    path: Option<String>,
    force: bool,
    ...
    let destination = resolve_install_destination(path, &config_dir)?;
    let outcome = install_bundle(&destination, &config_dir, force)?;
    let runtime_state = runtime_state_after_probe(outcome, &config_dir, probe).await?;
...
mod tests {
    ...
    fn extension_assets_share_the_bridge_contract() {
        assert_eq!(super::BROWSER_BRIDGE_RUNTIME_BUILD, "0.3.52");
    ...
    fn concurrent_process_installs_keep_secret_and_bundle_config_consistent() {
        ...
        let spawn_worker = |ready: &Path, attempting: &Path| {
            Command::new(&executable)
                .arg("--exact")
                .arg("browser_bridge::tests::cross_process_install_worker")
                .arg("--nocapture")
